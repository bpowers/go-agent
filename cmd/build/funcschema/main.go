...
func run() error {
	...
	tool := &MCPTool{
		...
		Description:  fmt.Sprintf("Function %s", *funcName), // TODO: Extract from doc comments
...
func generateToolDefFile(tool *MCPTool, funcName, paramTypeName, returnTypeName, inputFile, packageName string) error {
	// Marshal the tool definition to JSON
...
`, packageName, funcName, funcName, funcName, "`"+string(jsonBytes)+"`",
			funcName, funcName, funcName, returnTypeName)
	} else {
...
`, packageName, funcName, funcName, funcName, "`"+string(jsonBytes)+"`",
			funcName, funcName, paramTypeName, returnTypeName,
